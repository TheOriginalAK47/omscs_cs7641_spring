...
def plot_learning_curves_helper(model, features, labels, scoring_metric, plot_dir, problem_name):
	...
	plot = plot_learning_curve(model, model_type + " Learning Curves", \
						features, labels, axes=axes, ylim=(0.5, 1.01), \
...
def plot_opt_model_perf(model, features, labels, label_names, problem_name, plot_dir, axes=None):
	...
	model_pred_probs = model.predict_proba(features)
	...
	model_perf_array = [[str(model_acc), str(precision), str(recall), str(f1), str(round(roc_auc, 5))]]
...
def perf_random_search_for_best_hyper_params(clf, features, labels, param_dist, n_iter_search):
