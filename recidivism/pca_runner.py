...
plt.ylabel('Variance (%)') 
plt.title('Recidivism Dataset Explained Variance')
plt.savefig(plot_dir + problem_name + "_pca_variance_explained_by_num_components.png")

transformed_data = pd.DataFrame(pca.fit_transform(data_rescaled)[:, :10])

transformed_data.to_csv(data_dir + "pca/recidivism_data_pca_transformed.csv", index=False)